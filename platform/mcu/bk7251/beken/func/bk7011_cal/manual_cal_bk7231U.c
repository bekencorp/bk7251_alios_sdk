...
UINT32 manual_cal_load_default_txpwr_tab(UINT32 is_ready_flash)
{
    ...
    UINT8 org;
    int dist = 0, auto_mode = 0, use_dist = 0;;
    
    auto_mode = bk7011_is_rfcali_mode_auto();
    
    if(auto_mode) 
    {
        // no txpwr in flash, so use dist
        if(is_ready_flash == 0)
        {
            use_dist = 1;
    ...
    if(!(is_ready_flash & TXPWR_TAB_B_RD)) {
        ...
        if(use_dist) 
        {
            for(int i=0; i<WLAN_2_4_G_CHANNEL_NUM; i++)
            {
                txpwr_tab_ptr = &gtxpwr_tab_b[i];
                org = GET_TXPWR_GAIN(txpwr_tab_ptr);
                dist = g_def_dist_b;
                SET_TXPWR_GAIN(txpwr_tab_ptr, manual_cal_calc_dist(org, dist));
            }
        }
        MCAL_WARN("Load default txpwr for b:%p, dist\r\n", gtxpwr_tab_def_b, dist);
    }
    ...
    if(!(is_ready_flash & TXPWR_TAB_N_RD)) {
        ...
        MCAL_WARN("Load default txpwr for n40:%p, dist: %d\r\n", gtxpwr_tab_def_n_40, dist);
    }
    ...
    if(!(is_ready_flash & TXPWR_TAB_BLE)) {
        ...
        if(use_dist) 
        {
            for(int i=0; i<BLE_2_4_G_CHANNEL_NUM; i++)
            {
                txpwr_tab_ptr = &gtxpwr_tab_ble[i];
                org = GET_TXPWR_GAIN(txpwr_tab_ptr);
                dist = g_def_dist_ble;
        ...
        MCAL_WARN("Load default txpwr for ble:%p, dist:%d\r\n", gtxpwr_tab_def_ble, dist);
    }
    #endif
    return 0;
}
